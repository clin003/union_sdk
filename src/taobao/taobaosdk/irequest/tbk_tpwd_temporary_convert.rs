...
/// taobao.tbk.tpwd.temporary.convert( 淘宝客-推广者-淘口令解析&&转链（临时接口） )
/// 支持通过淘口令解析商品id，并提供对应的淘客转链接
// https://open.taobao.com/api.htm?docId=63323&docType=2&scopeId=16290
pub struct TbkTpwdTemporaryConvertRequest {
...
impl TbkTpwdTemporaryConvertRequest {
    pub fn new(
        session:&str,
        ...
        TbkTpwdTemporaryConvertRequest {
...
impl IRequest for TbkTpwdTemporaryConvertRequest {
    // 获取方法名
    fn get_method_name(&self) -> String {
        if self.session.len()>0{
            "taobao.tbk.sc.tpwd.temporary.convert".to_string()
        }else{
            "taobao.tbk.tpwd.temporary.convert".to_string()
        }       
    }
    // 获取请求参数
